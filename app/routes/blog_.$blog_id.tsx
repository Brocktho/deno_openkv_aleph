      ...
      {blog && (
        <Card clsxs={{ w: "w-full max-w-xl" }}>
          ...
          <p className="w-full flex flex-row items-center gap-3 px-3">
            <time
              dateTime={blog.created_at instanceof Date
                ? blog.created_at.toISOString()
                : `${blog.created_at}`}
            >
              Originally Created: {blog.created_at instanceof Date
                ? blog.created_at.toISOString()
                : `${blog.created_at}`}
            </time>
            <time
              dateTime={blog.updated_at instanceof Date
                ? blog.updated_at.toISOString()
                : `${blog.updated_at}`}
            >
              Last Updated: {blog.updated_at instanceof Date
                ? blog.updated_at.toISOString()
                : `${blog.updated_at}`}
            </time>
          </p>
